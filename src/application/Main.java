...
public class Main extends Application {
	...
	public void start(Stage stage) {
		try {
			...
			scene.setOnKeyReleased(new EventHandler<KeyEvent>() {
				public void handle(KeyEvent e) {
					...
					if (code == "O" || code == "P") {
						Image image;
						if (code == "O")
							image = new Image("pipeSmall.png");
						else
							image = new Image("pipeBig.png");
					...
					if (code == "UP" && !player.getFalling()) {
						Media jump = new Media(Main.class.getResource("/Jump.mp3").toString());
			...
			Media media = new Media(Main.class.getResource("/GroundTheme.mp3").toString());
			Media theEnd = new Media(Main.class.getResource("/GameOver.mp3").toString());
			...
			new AnimationTimer() {
				public void handle(long currentNanoTime) {
					...
					if (player.getGameOver()) {
						mp.setMute(true);
						theEndPlayer.play();
						theEndPlayer.setOnEndOfMedia(new Runnable() {
							@Override
							public void run() {
								theEndPlayer.stop();
							}
						...
						for (int i = 0; i < enemies.size(); i++) {
							...
							pipes.forEach(pipe -> enemy.collision(pipe));
						}
						// collision player - pipe
						pipes.forEach(pipe -> player.collision(pipe));
						...
						boxes.forEach(box -> {
							...
							if (box.getPowerUp() != null) {
								...
								for (int i = 0; i < pipes.size(); i++) {
									box.getPowerUp().collision(pipes.get(i));
								}
								player.collision(box.getPowerUp());
							}
						});
						// collision player - wall
						walls.forEach(wall -> player.collision(wall));
						...
						for (int i = 0; i < boxes.size(); i++) {
							if (boxes.get(i).getBoxCoin() != null && boxes.get(i).getBoxCoin().getX() < 0) {
								boxes.get(i).setBoxCoin(null);
							}
							if (boxes.get(i).getPowerUp() != null && boxes.get(i).getPowerUp().getX() < 0) {
								boxes.get(i).setPowerUp(null);
							}
						}
					}
